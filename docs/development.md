...
[Read First](architecture.md) - make sure you are familiar with the architecture and components of morty!
...
`java -jar target/morty-1.0-SNAPSHOT.jar`
...
docker build . -t morty:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 morty:latest
```
...
If something is not working, you can try one of the following commands to see what is deployed and their postmortemStatus.
...
If the pods say `Container image "morty:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
